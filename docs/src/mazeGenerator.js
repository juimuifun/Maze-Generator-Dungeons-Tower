...
function placeFloorEncounters(grid, width, length, floorNum, totalFloors, monsterDensity, miniBossFreq, trapDensity, secretFreq, customItems, guidePath, startPos, exitPos, autoGeneratedKeys, floorDoors, rng, pathWidth = 3, floorHeight = 5, foundationHeight = 5, floorGap = 5) {
    ...
    for (let i = 0; i < secretCount && deadEndTiles.length > 0; i++) {
        ...
        const isBreakableGuard = !isDoorGuard && (rng() > 0.5);

        if (isBreakableGuard) {
            grid[deadEnd.y][deadEnd.x] = 'BREAKABLE';
            placedEntities.push({
                type: 'BREAKABLE',
                x: deadEnd.x,
                y: deadEnd.y,
            ...
            if (isDoorGuard) {
                const secretKeyId = `key_secret_${deadEndCoord.toLowerCase()}_f${floorNum}`;
                const secretKeyObj = {
                    id: secretKeyId,
                    type: 'KEY',
                    floorRule: `floor-${floorNum}`,
                    targetDoor: { x: deadEnd.x, y: deadEnd.y, gridNotation: deadEndCoord }
                };
                autoGeneratedKeys.push(secretKeyObj);
                floorDoors.push({
                    id: `door-secret-${deadEndCoord.toLowerCase()}-f${floorNum}`,
                    position: { x: deadEnd.x, y: deadEnd.y, gridNotation: deadEndCoord },
                    floor: floorNum,
                    targetType: 'SECRET',
