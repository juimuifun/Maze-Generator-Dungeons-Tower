...
export function generateMazeV2(config, customRooms = [], customItems = []) {
    ...
    for (let f = 0; f < floors; f++) {
        ...
        const placedRooms = placeSpecialRoomsOnFloor(
            ...
            pathWidth, floorHeight, foundationHeight, floorGap, miniBossInterval
        );
...
function placeSpecialRoomsOnFloor(grid, width, length, floorNum, totalFloors, rooms, customItems, autoGeneratedKeys, floorDoors, rng, pathWidth = 3, floorHeight = 5, foundationHeight = 5, floorGap = 5, miniBossInterval = 2) {
    const placed = [];
    ...
    rooms.forEach(room => {
        ...
        if (mode === 'all') {
            ...
            else if (room.type === 'SECRET' || room.type === 'TRAP') matchesFloor = true;
        } else if (mode === 'custom') {
        ...
        if (bestPos) {
            ...
            const fullRoomName = room.name ? `${room.name} [${centerNotation}]` : `${room.type} [${centerNotation}]`;
            const roomId = room.name ? room.name.toLowerCase().replace(/\s+/g, '_') : `${room.type.toLowerCase()}_${centerNotation.toLowerCase()}`;

            // 3. Carve ONLY ONE single doorway entrance on the room border (top edge center)
            ...
            // Connect outer side of door to maze path network
            const outerY = (bestPos.y > 1) ? doorY - 1 : doorY + 1;
            if (outerY >= 0 && outerY < length && grid[outerY][doorX] === 0) {
                grid[outerY][doorX] = 1;
            }
