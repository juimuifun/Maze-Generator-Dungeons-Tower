...
export function generateMazeV2(config, customRooms = [], customItems = []) {
    ...
    for (let f = 0; f < floors; f++) {
        ...
        generatedFloors.push({
            ...
            guidePath: guidePath,
            startPos: { ...startPos, gridNotation: toGridNotation(startPos.x, startPos.y) },
            exitPos: { ...exitPos, gridNotation: toGridNotation(exitPos.x, exitPos.y) }
        });
    }
...
function placeSpecialRoomsOnFloor(grid, width, length, floorNum, totalFloors, rooms, customItems, autoGeneratedKeys, floorDoors, rng) {
    const placed = [];
    ...
    rooms.forEach(room => {
        ...
        if (bestPos) {
            ...
            placed.push({
                ...
                name: fullRoomName,
                type: room.type,
                center: { x: centerX, y: centerY, gridNotation: centerNotation },
                bounds: { x: bestPos.x, y: bestPos.y, width: rw, height: rh },
                door: requiredKeyObj ? { position: { x: doorX, y: doorY, gridNotation: doorNotation }, requiredKey: requiredKeyObj } : null
            });
        }
    });
...
function placeFloorEncounters(grid, width, length, floorNum, totalFloors, monsterDensity, miniBossFreq, trapDensity, secretFreq, customItems, guidePath, startPos, exitPos, autoGeneratedKeys, floorDoors, rng) {
    ...
    for (let i = 0; i < secretCount && deadEndTiles.length > 0; i++) {
        ...
        if (deadEnd.frontTile && grid[deadEnd.frontTile.y][deadEnd.frontTile.x] === 1) {
            ...
            grid[deadEnd.frontTile.y][deadEnd.frontTile.x] = isDoorGuard ? 'DOOR' : 'BREAKABLE';
        ...
        placedEntities.push({ type: 'SECRET', x: deadEnd.x, y: deadEnd.y, gridNotation: deadEnd.gridNotation });
    }
    ...
    for (let i = 0; i < trapCount; i++) {
        if (i % 2 === 0 && guidePath.length > 6) {
            ...
            if (grid[chokepoint.y][chokepoint.x] === 1 && getDist(chokepoint.x, chokepoint.y, startPos.x, startPos.y) > 4) {
                ...
                placedEntities.push({ type: 'TRAP', style: 'chokepoint', x: chokepoint.x, y: chokepoint.y, gridNotation: toGridNotation(chokepoint.x, chokepoint.y) });
            }
        } else if (deadEndTiles.length > 0) {
            const deadEnd = deadEndTiles.pop();
            grid[deadEnd.y][deadEnd.x] = 'TRAP';
            placedEntities.push({ type: 'TRAP', style: 'lure_deadend', x: deadEnd.x, y: deadEnd.y, gridNotation: deadEnd.gridNotation });
        }
    }
    ...
    for (let i = 0; i < monsterCount && emptyPathTiles.length > 0; i++) {
        ...
        placedEntities.push({ type: 'MONSTER', x: tile.x, y: tile.y, gridNotation: tile.gridNotation });
    }
    ...
    if (deadEndTiles.length > 0 && rng() > 0.4) {
        ...
        placedEntities.push({ type: 'PORTAL', x: portalDeadEnd.x, y: portalDeadEnd.y, gridNotation: portalDeadEnd.gridNotation });
    }
    ...
    customItems.forEach(item => {
        ...
        if (matchesFloor) {
            placedEntities.push({ id: item.id, name: item.name, keyId: item.keyId, type: 'QUEST_ITEM', mode: item.mode, targetRoomId: item.lockedRoomId });
        }
    });
