...
function placeFloorEncounters(grid, width, length, floorNum, totalFloors, monsterDensity, miniBossFreq, trapDensity, secretFreq, customItems, guidePath, startPos, exitPos, autoGeneratedKeys, floorDoors, rng, pathWidth = 3, floorHeight = 5, foundationHeight = 5, floorGap = 5) {
    ...
    while (secretsPlaced < targetSecretCount && deadEndTiles.length > 0) {
        ...
        if (isDoorGuard) {
            const secretKeyId = `key_secret_${doorCoord.toLowerCase()}_f${floorNum}`;
            const secretKeyObj = {
                id: secretKeyId,
                type: 'KEY',
                floorRule: `floor-${floorNum}`,
                targetDoor: { x: doorPos.x, y: doorPos.y, gridNotation: doorCoord }
            };
            autoGeneratedKeys.push(secretKeyObj);
            floorDoors.push({
                id: `door-secret-${doorCoord.toLowerCase()}-f${floorNum}`,
                position: { x: doorPos.x, y: doorPos.y, gridNotation: doorCoord },
                floor: floorNum,
                targetType: 'SECRET',
